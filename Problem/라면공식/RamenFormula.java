...
// 예찬이가 라면을 끓이는 횟수 N과 i (1 <= i <= N)번째로 라면을 끓일 때의 라면 계수
// A_i, 기본 물의 양, B_i, 끓일 라면 수 X_i가 주어질 때, 예찬이를 위해 라면 공식에 따라 필요한 물의 양 W_i을 계산해 보자.

// 입력
// 첫째 줄에 예찬이가 라면을 끓이는 횟수 N이 주어진다.
// 둘째 줄부터 N+1번째 줄까지는 i + 1 (1 <= i <= N)번째 줄에
// A_i, B_i, X_i가 공백으로 구분되어 주어진다.
// 입력으로 주어지는 모든 수는 정수이다.

// 출력
// N개의 줄을 출력하라. 이 중
// i (1 <= i <= N)번째 줄에 i번째로 라면을 끓일 때 라면 공식에 따라 필요한 물의 양 W_i를 출력하라.

// 제한
// 1 <= N <= 100
// 100 <= A_i, B_i <= 1,000 (1 <= i <= N)
// 1 <= X_i <= 100 (1 <= i <= N)
...
public class RamenFormula {
    public static void main(String[] args) {
        ...
        for(int i = 0; i < n; i++) {
            ...
            int needWater = ramen * (cnt - 1) + water;
