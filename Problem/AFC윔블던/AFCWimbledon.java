...
// 이때부터 10년동안 임시로 근처 클럽이었던 크리스털 팰리스와 경기장을 공유하기 시작했고 점점 재정적인 어려움을 겪었다. 결국 2002년 윔블던에서 약 90km정도 떨어진 밀턴 킨스로 연고지를 이전하는 초유의 사태가 벌어졌으며, 2004년 윔블던 FC는 파산하게 된다.
...
// 두 팀이 공식 경기에서 만나려면 FA Cup밖에 기회가 없었다. 하지만, FA Cup은 하부 리그 팀은 낮은 라운드부터 대회에 참가하고,
// 대진은 추첨이므로 두 팀이 만나는 것은 불가능하다 생각했다.
...
// MK 돈스는 2004년부터 2시즌(06-07, 07-08)을 제외하고는 항상 3부리그에서 활동했다.
// 이제 MK 돈스가 강등당하거나 AFC 윔블던이 승격을 하면 리그에서 두 팀간의 경기를 볼 수 있다.
...
public class AFCWimbledon {
    public static void main(String[] args) {
        ...
        if(sum >= sub && (sum + sub) % 2 == 0){ // sum + sub 합이 짝수인 경우
            long a = (sum + sub) / 2; // 득점을 많이 한 쪽
            long b = (sum - sub) / 2; // 득점을 적게 한 쪽
