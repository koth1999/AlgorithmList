package Problem.MiniFatasyWar;

// 넥슨지티에서는 최근 SRPG "슈퍼 판타지 워"의 후속작으로 "미니 판타지 워"를 출시하였다. 전편과 마찬가지로, 미니 판타지 워에서도 각 캐릭터의 전투력을 계산하여야 한다.
...
// 공격력은 0보다 작은 경우 0으로 간주한다. 방어력은 음수 여부에 상관하지 않는다.
...
// 다음 T 행에 걸쳐 각각 8개의 정수가 주어진다. 처음 네 개의 정수는 각각 캐릭터의 기본 HP, MP, 공격력, 방어력이다. 이 값들은 1 이상 999 이하로 주어진다.
// 다음 네 개의 정수는 각각 캐릭터의 장비에 의해 증감되는 HP, MP, 공격력, 방어력이다. 이 값들은 -999 이상 999 이하로 주어진다.
...
public class MiniFantasyWar {
    public static void main(String[] args) {
        ...
        for(int i = 0; i < t; i++) {
            ...
            int hpPlus = sc.nextInt();
            int mpPlus = sc.nextInt();
            int attackPlus = sc.nextInt();
            int defensePlus = sc.nextInt();
            ...
            if(attack + attackPlus < 0) {
                attack = 0;
            } else {
                attack += attackPlus;
            }

            int power = 1 * hp + 5 * mp + 2 * attack + 2 * (defense + defensePlus);
