...
// 이때 점수가 부여되는 공식적인 기술은 '한손 코딩', '노룩 코딩', '폰코딩'으로 총 3가지이고, 각 기술들에는 난이도가 다르게 부여된다.
// 심사 방식은 다음과 같다.
...
// 입력
// 첫째 줄에 세 가지 기술의 난이도 A, B, C가 '한손 코딩', '노룩 코딩', '폰코딩' 순서대로 공백을 사이에 두고 주어진다.
// (0 <= A, B, C <= 1,000)
// 둘째 줄에 참가한 동아리의 수 N이 주어진다. (1 <= N <= 1,000)
// 셋째 줄부터
// 3N개의 줄에 걸쳐 세 줄마다 각 동아리의 기술 사용 정보가 주어진다.
// 세 개의 줄에는 각 줄마다 동아리를 구성하는 각 동아리원이 사용한 기술의 횟수
// a, b, c가 '한손 코딩', '노룩 코딩', '폰코딩' 순서대로 공백을 사이에 두고 주어진다.
// (0 <= a, b, c <= 100)
...
public class StreetCodingFighter {
    public static void main(String[] args) {
        ...
        int A = sc.nextInt(); // 한손 코딩 난이도
        int B = sc.nextInt(); // 노룩 코딩 난이도
        int C = sc.nextInt(); // 폰 코딩 난이도
        ...
        for(int i = 0; i < n; i++) {
            int sum = 0;
            for(int j = 0; j < 3; j++) {
                int a = sc.nextInt(); // 한손 코딩 점수
                int b = sc.nextInt(); // 노룩 코딩 점수
                int c = sc.nextInt(); // 폰 코딩 점수
